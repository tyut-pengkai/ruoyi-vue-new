<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          v-hasPermi="['system:appLogininfor:export']"
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      listAppLogininfor(this.queryParams).then((response) => {
        this.appLogininforList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getAppLogininfor(infoId).then((response) => {
      ...
      this.$refs["form"].validate((valid) => {
        if (valid) {
          if (this.form.infoId != null) {
            updateAppLogininfor(this.form).then((response) => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addAppLogininfor(this.form).then((response) => {
    ...
    handleExport() {
      ...
      this.$modal
        ...
        .then(() => {
          ...
          return exportAppLogininfor(queryParams);
        })
        .then((response) => {
