...
:::tip 为了简化安装步骤，接下来的步骤将使用UPUPW面板安装 UPUPW面板下载地址：[https://php.upupw.net/](https://php.upupw.net/)
:::
...
Mysql、Redis、Nginx这三个软件已经集成在在UPUPW面板中，我们只需安装UPUPW面板即可 注意选择版本时选择Nginx版本进行安装，如图所示：
![image](./1.jpg)
![image](./2.jpg)  
...
编辑`application-config.yml`，将数据库用户名更改为`root`，`数据库密码`更改为上一步获取的数据库的密码，此处请注意`jdbc:mysql://127.0.0.1:3306/hywlyz?useUnicode=`
中的`hywlyz`为数据库名，若与您不一致请注意修改
