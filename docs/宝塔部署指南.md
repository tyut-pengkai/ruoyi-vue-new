...
nohup java -jar ruoyi-admin.jar --spring.config.location=application.yml,application-druid.yml > app.log 2>&1 &
...
### 查看应用日志
```bash
docker logs beautyclub-backend
```

### 停止应用
```bash
docker stop beautyclub-backend
```

### 重启应用
```bash
docker restart beautyclub-backend
```

### 进入容器
```bash
docker exec -it beautyclub-backend sh
```
