...
CREATE TABLE `payment_package` (
  ...
  `hours` int(11) NOT NULL COMMENT '套餐时长（小时）',
  `price` decimal(10, 2) NOT NULL COMMENT '价格（美元）',
  `currency` varchar(20) NOT NULL COMMENT '币种(USD=美金, CNY=人民币)',
...
CREATE TABLE `payment_order` (
  ...
  `package_id` bigint(20) DEFAULT NULL COMMENT '套餐ID（自定义时可为空）',
  `package_name` varchar(100) DEFAULT NULL COMMENT '套餐名称',
  `package_hours` int(11) DEFAULT NULL COMMENT '套餐时长（小时）',
  `amount` decimal(10, 2) NOT NULL COMMENT '订单金额（美元）',
  `currency` varchar(20) NOT NULL COMMENT '币种(USD=美金, CNY=人民币)',
  `status` char(1) NOT NULL DEFAULT '0' COMMENT '订单状态（0=待支付, 1=已完成, 2=已取消, 3=支付失败）',
  `payment_channel` varchar(20) DEFAULT NULL COMMENT '支付渠道（Stripe, PayPal等）',
  `third_party_no` varchar(128) DEFAULT NULL COMMENT '第三方交易号',
