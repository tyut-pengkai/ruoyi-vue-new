...
<!--      </el-table-column>-->
    ...
    <el-dialog :title="title" :visible.sync="open" append-to-body width="500px">
      <el-form ref="form" :model="form" :rules="rules">
        <el-form-item label="文件名" prop="name">
          <el-input v-model="form.name" placeholder="请输入文件名" maxlength="30" show-word-limit/>
        </el-form-item>
  ...
  methods: {
    ...
    handleCheckTokenChange(row) {
      ...
      this.$modal
        ...
        .then(function () {
          return updateGlobalFile({'id': row.id, 'checkToken': row.checkToken});
        })
        .then(() => {
    ...
    handleCheckVipChange(row) {
      ...
      this.$modal
        .confirm("确认要将\"" + row.name + "\"置为" + text + '吗？')
        .then(function () {
          return updateGlobalFile({'id': row.id, 'checkVip': row.checkVip});
        })
        .then(() => {
          this.$modal.msgSuccess("置为" + text + "成功");
        })
