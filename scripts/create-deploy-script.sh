...
# 创建手动部署脚本
# 用于在服务器上直接生成manual-deploy.sh文件

set -e

# 目标文件路径
SCRIPT_PATH="/www/wwwroot/beautyclub/manual-deploy.sh"
...
show_result() {
    ...
    echo "更新代码: ./manual-deploy.sh -b ${GIT_BRANCH}"
}
...
echo "✅ 手动部署脚本创建成功: ${SCRIPT_PATH}"
echo "现在可以使用以下命令进行部署："
echo "  ./manual-deploy.sh -b main"
echo "  ./manual-deploy.sh -b develop"
echo "  ./manual-deploy.sh -h  # 查看帮助" 