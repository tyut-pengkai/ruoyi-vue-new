<template>
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          icon="el-icon-plus"
          size="mini"
          @click="openBindDeviceUserDialog"
          v-hasPermi="['device:info:bindUser']"
        >添加设备</el-button>
      </el-col>
  ...
  <el-dialog title="添加设备" :visible.sync="bindDeviceDialogVisible" width="400px">
  <el-form :model="bindDeviceUserForm" label-width="100px">
    <el-form-item label="设备">
      <el-input v-model="bindDeviceUserForm.deviceMxcCode" placeholder="请输入设备编码或者mxc地址"></el-input>
    </el-form-item>
  </el-form>
  <div slot="footer" class="dialog-footer">
    <el-button @click="bindDeviceDialogVisible = false">取 消</el-button>
    <el-button type="primary" @click="handleBindDeviceUser">确 定</el-button>
  </div>
  </el-dialog>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        mxcAddr: [
          { required: true, message: "mxc地址不能为空", trigger: "blur" }
        ],
      },
  ...
  methods: {
    ...
    /** 打开绑定设备的对话框 */
    openBindDeviceUserDialog() {
      this.bindDeviceUserForm.deviceMxcCode = '';
      this.bindDeviceDialogVisible = true;
    },
    /** 绑定设备 */
    handleBindDeviceUser() {
      if (!this.bindDeviceUserForm.deviceMxcCode) {
        this.$message.error('请填写设备编码或mxc地址');
        return;
      }
      // 调用API
      bindDeviceUser(this.bindDeviceUserForm).then(res => {
