<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="86px">
      ...
      <!-- <el-form-item label="毛胚重量参数" prop="blackWight">
        <el-input
          v-model="queryParams.blackWight"
          placeholder="请输入毛胚重量参数"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
      <el-form-item label="废料重参数" prop="scrapWight">
        <el-input
          v-model="queryParams.scrapWight"
          placeholder="请输入废料重参数"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
      ...
      <el-form-item label="酸洗" prop="sprayPick">
        <el-input
          v-model="queryParams.sprayPick"
          placeholder="请输入酸洗"
          clearable
          @keyup.enter.native="handleQuery"
        />
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['system:quoteparam:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:quoteparam:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:quoteparam:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:quoteparam:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="quoteparamList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
      <el-table-column label="id" align="center" prop="id" />
      <el-table-column label="关联客户id" align="center" prop="customerId" />
      <el-table-column label="毛胚重量参数" align="center" prop="blackWight" />
      <el-table-column label="废料重参数" align="center" prop="scrapWight" />
      ...
      <el-table-column label="酸洗" align="center" prop="sprayPick" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="毛胚重量参数" prop="blackWight">
          <el-input v-model="form.blackWight" placeholder="请输入毛胚重量参数" />
        </el-form-item>
        <el-form-item label="废料重参数" prop="scrapWight">
          <el-input v-model="form.scrapWight" placeholder="请输入废料重参数" />
        </el-form-item>
        <el-form-item label="数割下料费" prop="cutMaterial">
        ...
        <el-form-item label="酸洗" prop="sprayPick">
          <el-input v-model="form.sprayPick" placeholder="请输入酸洗" />
        </el-form-item>
        <el-form-item label="喷塑" prop="sprayPlastic">
...
<script>
import { listQuoteparam, getQuoteparam, delQuoteparam, addQuoteparam, updateQuoteparam } from "@/api/system/quoteparam";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listQuoteparam(this.queryParams).then(response => {
        this.quoteparamList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getQuoteparam(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateQuoteparam(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addQuoteparam(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除报价参数编号为"' + ids + '"的数据项？').then(function() {
        return delQuoteparam(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/quoteparam/export', {
        ...this.queryParams
      }, `quoteparam_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
